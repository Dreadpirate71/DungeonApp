...
namespace Dungeon_App
{
    internal class Combatant
    {
       ...
       public string Name
        ...
        public void DoBattle(Monster monster, Character character, Weapon weapon, int monsterMultiplier, Hero hero)
        {
            bool exitRoom = false;
            ...
            do
            {
                ...
                switch (userChoiceRoom)
                {
                    case ConsoleKey.A:
                        ...
                        Console.WriteLine("{0}, you did {1} damage! {2} now has {3} health.\n", character.Name, character.HitDamage, monster.Name, monster.CurrentHealth);
                        Console.ForegroundColor = ConsoleColor.Blue;
                        Console.WriteLine("{0} did {1} damage! {2}, you now have {3} health.\n", monster.Name, monster.HitDamage, character.Name, character.CurrentHealth);
                        if (character.CurrentHealth <= 0)
                        {
                            character.CurrentHealth = 0;
                            character.TotalCharacterLoot -= monster.Loot;
                            if (character.TotalCharacterLoot < 0)
                            {
                                character.TotalCharacterLoot = 0;
                            }
                            Console.ForegroundColor = ConsoleColor.Red;
                        ...
                        else if (monster.CurrentHealth <= 0)
                        {
                            ...
                            if (character.TotalCharacterExperience / 100 != character.HeroLevel)
                            {
                    ...
                    case ConsoleKey.R:
                        Console.ForegroundColor = ConsoleColor.Cyan;
                        Console.WriteLine("You have chosen to run away from {0} and live to fight another day!\n", monster.Name);
                        break;
                    case ConsoleKey.X:
        ...
        public bool DoBattle(Monster monster, Character character, Weapon weapon, bool exitRoom, Hero hero)
        {
            var randomNumWeapon = new Random();
            var randomNumMonsterDamage = new Random();
            var randomNumMonsterBonus = new Random();
            ...
            do
            {
                ...
                switch (userChoiceRoom)
                {
                    case ConsoleKey.A:
                        ...
                        {
                            character.TotalCharacterLoot += monster.Loot;
                            character.TotalCharacterExperience += monster.Experience;
                            if (character.TotalCharacterExperience/100 != character.HeroLevel)
                            {
                                hero.ElevateHeroLevel(character);
                            }
